...
void socket_disconnect_handler(int socket_num, Server *server_data)
{
    ...
    */
    ...
    for (i = 0; i < server_data->sockets_count && server_data->sockets_arr[0][i] != socket_num ; i++)
        ;
    
    for (; i < server_data->sockets_count; i++){
        if (server_data->sockets_arr[0] != NULL)
            server_data->sockets_arr[0][i] = server_data->sockets_arr[0][i + 1];
